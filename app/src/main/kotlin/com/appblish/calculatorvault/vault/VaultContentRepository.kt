...
interface VaultContentRepository {
     ...
     * There is no lazy backfill — a missing strip just means the seekbar shows only its time
     * code. Default null so in-memory fakes keep working.
     */
    suspend fun openPreviewStrip(itemId: String): ByteArray? = null

    /**
     * Persist [stripBytes] as [itemId]'s scrub-preview storyboard, **encrypted** — plaintext
     * frames must never touch disk (same mandate as [saveThumbnail]). Generated by the device
     * implementation at hide-time in the same pass as the grid thumbnail. Default no-op for fakes.
     */
    suspend fun savePreviewStrip(
        itemId: String,
