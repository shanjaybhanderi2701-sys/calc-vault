...
/**
 * The APP-244 memory bound (moved into the kit by APP-438): the decoded-strip LRU must never exceed
 * its byte budget (no OOM with a huge library), must evict least-recently-used first (a revisited
 * 30-item grid stays fully cached), and must honor point eviction (delete/restore) and [clear].
 */
